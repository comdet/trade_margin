...
class BinanceOrderBalanceManager(AbstractOrderBalanceManager):
    ...
    def create_order(self, **params):
        return self.binance_client.create_margin_order(**params)
                ...
                cache_balances.update(
                    {
                        ...
                        for currency_balance in self.binance_client.get_margin_account()["userAssets"]
                    }
                )
...
class BinanceAPIManager:
    ...
    def create_manager(config: Config, db: Database, logger: Logger) -> "BinanceAPIManager":
        return BinanceAPIManager._common_factory(
            config, db, logger, lambda client, cache: BinanceOrderBalanceManager(
                logger, config, client, cache)
        )
    ...
    def create_manager_paper_trading(
        ...
        manager = BinanceAPIManager._common_factory(
            ...
            lambda client, cache: PaperOrderBalanceManager(
                config.BRIDGE.symbol, client, cache, initial_balances or {
                    config.BRIDGE.symbol: 100.0}
            ),
        )
    ...
    def get_account(self):
        ...
        """
        return self.binance_client.get_margin_account()
    ...
    def get_ask_price(self, ticker_symbol: str):
        ...
        if price is None and ticker_symbol not in self.cache.non_existent_tickers:
            try:
                ticker = self.binance_client.get_orderbook_ticker(
            ...
            try:
                ...
                if self._should_cancel_order(order_status):
                    ...
                    while cancel_order is None:
                        cancel_order = self.binance_client.cancel_margin_order(
    ...
    def _adjust_bnb_balance(self, origin_coin: Coin, target_coin: Coin):
        ...
        # The discount is only applied if we have enough BNB to cover the fee
        amount_trading = self._buy_quantity(
        ...
        fee_amount_bnb_ceil = math.ceil(
            (fee_amount_bnb * self.config.AUTO_ADJUST_BNB_BALANCE_RATE - bnb_balance) * 10 ** alt_tick) / float(10 ** alt_tick)
        ...
        min_qty_for_min_notinal = math.ceil(
            (min_notional / bnb_price) * 1.01 * 10 ** alt_tick) / float(10 ** alt_tick)

        buy_quantity = max(min_qty, fee_amount_bnb_ceil,
                           min_qty_for_min_notinal)

        self.logger.info(
            f"Needed/available BNB balance: {fee_amount_bnb}/{bnb_balance}, buy quantity: {buy_quantity}...")

        is_bnb_enabled = "BNB" in self.config.SUPPORTED_COIN_LIST

        self.retry(self._buy_alt, Coin("BNB", enabled=is_bnb_enabled),
                   target_coin, bnb_price, buy_quantity)
    ...
    def _buy_quantity(
        ...
        target_balance = target_balance or self.get_currency_balance(
            target_symbol)
        from_coin_price = from_coin_price or self.get_buy_price(
            origin_symbol + target_symbol)
    ...
    def _buy_alt(self, origin_coin: Coin, target_coin: Coin, buy_price: float, buy_quantity: float = None):  # pylint: disable=too-many-locals
        ...
        order_guard.set_order(
            origin_symbol, target_symbol, int(order["orderId"]))
...
class PaperOrderBalanceManager(AbstractOrderBalanceManager):
    ...
    def _write_persist(self):
        with open(self.PERSIST_FILE_PATH, "w") as json_file:
            json.dump({"balances": self.balances,
                       "fake_order_id": self.fake_order_id}, json_file)
    ...
    def make_order(
        ...
        if side == Client.SIDE_SELL:
            fees = self.manager.get_fee(
                Coin(symbol_base), Coin(self.bridge), True)
            self.balances[self.bridge] = self.get_currency_balance(
                self.bridge) + quote_quantity * (1 - fees)
            self.balances[symbol_base] = self.get_currency_balance(
                symbol_base) - quantity
        else:
            fees = self.manager.get_fee(
                Coin(symbol_base), Coin(self.bridge), False)
            self.balances[self.bridge] = self.get_currency_balance(
                self.bridge) - quote_quantity
            self.balances[symbol_base] = self.get_currency_balance(
                symbol_base) + quantity * (1 - fees)
